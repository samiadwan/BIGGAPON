                                            ...
                                            <optgroup label="{{ $category->category_name }}">
                                                @foreach ($category->sub_categories as $sub_category)
                                                    <option value="{{ $sub_category->id }}"
                                                        {{ old('category') == $sub_category->id ? 'selected' : '' }}>
                    ...
                    <div itemscope itemtype="http://schema.org/Product"
                        ...
                        <div class="caption">
                            <h4><a href="{{ route('single_ad', [$ad->id, $ad->slug]) }}"
                                    title="{{ $ad->title }}"><span itemprop="name">{{ str_limit($ad->title, 40) }}
                                    </span></a></h4>
                            <a class="price text-muted" href="{{ route('listing', ['category' => $ad->category->id]) }}">
                                <i class="fa fa-folder-o"></i> {{ $ad->category->category_name }} </a>

                            @if ($ad->city)
                                <a class="location text-muted" href="{{ route('listing', ['city' => $ad->city->id]) }}">
                                    <i class="fa fa-location-arrow"></i> {{ $ad->city->city_name }} </a>
                            @endif
                            <p class="date-posted text-muted"> <i class="fa fa-clock-o"></i>
                                {{ $ad->created_at->diffForHumans() }}</p>
                            <p class="price"> <span itemprop="price" content="{{ $ad->price }}">
                                    {{ ads_price_ng($ad->price, $ad->is_negotiable) }} </span></p>
                            <link itemprop="availability" href="http://schema.org/InStock" />
                        </div>

                        @if ($ad->price_plan == 'premium')
                            <div class="label-premium" data-toggle="tooltip" data-placement="top"
                                title="{{ ucfirst($ad->price_plan) }} ad"><i class="fa fa-star-o"></i> </div>
                        @endif
                    </div>
                </div>
            @endforeach
                            ...
                            <div class="caption">
                                ...
                                <a class="price text-muted"
                                    href="{{ route('listing', ['category' => $ad->category->id]) }}"> <i
                                        class="fa fa-folder-o"></i> {{ $ad->category->category_name }} </a>
                                @if ($ad->city)
                            ...
                            <section class="post">
                                <div class="row">
                                    ...
                                    <div class="col-md-8">
                                        ...
                                        <div class="clearfix">
                                            @if ($post->author)
                                                <p class="author-category">By <a
                                                        href="{{ route('author_blog_posts', $post->author->id) }}">{{ $post->author->name }}</a>
                                                </p>
                                            @endif
                                        ...
                                        <p class="intro"> {{ str_limit(strip_tags($post->post_content), 120) }} <a
                                                href="{{ route('blog_single', $post->slug) }}">@lang('app.continue_reading')</a></p>
                                        <p></p>
                                    </div>
                                </div>
                            </section>
